function [ labels, file_list ] = load_stip_data( params, file_type )
%LOAD_STIP_DATA Load STIP data
%   Assume that the stip files have similar hierachy with UCF 101
%   Inputs:
%       stip_dir - directory of gzipped STIP descriptors
%       tt_list_dir - directory of train/test file list
%       file_pattern - pattern of train/test list, such as 'train*' or 'test*'
%       num_feats - number of features extracted from each stip file
%
%   Outputs:
%       L -labels for STIP features
...
% extract STIP data and set labels
%try
%	matlabpool close;
%catch exception
%end
...
parfor i=1:length(tt_list)
    ...
    clip_name=regexprep(tt_list{i},'\.avi$',''); % get video clip name
    clip_name=regexprep(clip_name,'.*/','');
    stip_file=fullfile(act_dir,[clip_name,'.stip.gz']);
    ...
    [HOG,HOF,MBHx,MBHy]=extract_stip_feats(stip_file,params);
    % To save memory, write STIP features into txt files instead of stored in memory
